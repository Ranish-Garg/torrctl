                ...
                match msg.unwrap() {
                    ...
                    Message::Piece {
                        ...
                        if (hashmp.contains_key(&index)) {
                            let pb: &mut piece_buffer = hashmp.get_mut(&index).unwrap();
                            let len = block.len();
                            ...
                            for i in 0..pb.received.len() {
                                if !pb.received[i]
                                    && !pb.requested[i]
                                    && active_requests < MAX_REQUEST
                                {
                                    let offset = (i * BLOCK_SIZE) as u32;

                                    let blocksz = if i == pb.received.len() - 1
                                        && pb.data.len() % BLOCK_SIZE != 0
                                    {
                                    ...
                                    request_block(&mut stream, index, offset, blocksz).ok();
                                    pb.requested[i] = true;
                                    active_requests += 1;
                                }
                            }

                            if checkpiececompletion(&index, pb) == Some(true) {
                    ...
                    if let Some(piece) = pick_piece(&peer_piece, &bitfield) {
                        ...
                        if !inprogress.contains_key(&piece) {
                            ...
                            for item in 0..num_blocks {
                                let blocksz =
                                    if item == num_blocks - 1 && piece_size % BLOCK_SIZE != 0 {
                                        (piece_size % BLOCK_SIZE) as u32
                                    } else {
                                        BLOCK_SIZE as u32
                                    };
                                if let Some(pb) = inprogress.get_mut(&piece) {
                                    if pb.received[item] == false {
                                        let offset = (item * BLOCK_SIZE) as u32;

                                        let count = pb.requested.iter().filter(|&&x| x).count();

                                        if count < MAX_REQUEST {
                                            request_block(&mut stream, piece, offset, blocksz).ok();
...
fn checkpiececompletion(index: &u32, piece_buffer: &mut piece_buffer) -> Option<bool> {
    let received_vec = &piece_buffer.received;
